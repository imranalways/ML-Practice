...
namespace MLNetDemo01.Part13
{
    public class Demo
    {
        public static void Execute()
        {
            ...
            var crossValidationResult = context.Regression.CrossValidate(trainingData, pipeline,numberOfFolds:5,labelColumnName:"Salary");

            var allMatrices = crossValidationResult.Select(x => x.Metrics);
            ...
            var bestPerformance = allMatrices.OrderByDescending(x => x.RSquared).FirstOrDefault();
            var bestPerformanceIndex = allMatrices.ToList().IndexOf(bestPerformance);
            var bestModel = allModels.ElementAt(bestPerformanceIndex);

            Console.WriteLine("Best Performace ");
            Console.WriteLine($"Best Model: {bestPerformance.RSquared:0.00}");
            Console.WriteLine($"Best Model Index: {bestPerformanceIndex}");
            Console.WriteLine("=============================================");
            Console.WriteLine("Average Rate");

            Console.WriteLine($"R^2: {allMatrices.Average(x=>x.RSquared):0.00}");
            Console.WriteLine($"MA Error: {allMatrices.Average(x => x.MeanAbsoluteError):0.00}");
            Console.WriteLine($"MS: {allMatrices.Average(x => x.MeanSquaredError):0.00}");
            Console.WriteLine($"RMS: {allMatrices.Average(x => x.RootMeanSquaredError):0.00}");
            Console.WriteLine($"Loss Function: {allMatrices.Average(x => x.LossFunction):0.00}");
